...
var _ = Describe("List Versions command", func() {
	It("is registered on the root command", func() {
		cmd, _, err := rootCmd().Find([]string{"list-versions"})

		Expect(err).ToNot(HaveOccurred())
		Expect(cmd.Name()).To(Equal("list-versions"))
	})
	...
	DescribeTable("rejects invalid app ID values when no bundle identifier given",
		...
		Entry("app ID negative", "-100", "positive number"),
	)

	It("accepts positive app ID values", func() {
		cmd := ListVersionsCmd()
		Expect(cmd.Flags().Set("app-id", "987654")).To(Succeed())
